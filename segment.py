...
class Curve:
   def __init__(self, vertices: List[Vector], thickness: float = 0.05, side_bias: float = 0.0, 
      ...
      self.angle_offs = [math.radians(ao) for ao in [angle_offs0, angle_offs1]]
   ...
   def _make_intro(self):
      ...
      self.obj.keyframe_insert(data_path='["load_factor"]', frame=to_frame(self.intro.start))
      self.obj["load_factor"] = 1.0
      self.obj.keyframe_insert(data_path='["load_factor"]', frame=to_frame(self.intro.stop))
...
class Segment(Curve):
   def __init__(self, vert0: Vector, vert1: Vector, thickness: float = 0.05, side_bias: float = 0.0, 
                angle_offs0: float = 0.0, angle_offs1: float = 0.0, intro: Interval = None, 
                outro: Interval = None):
      super().__init__([vert0, vert1], thickness, side_bias, angle_offs0, angle_offs1, intro, outro)
   ...
   def _build(self):
      ...
      for i in range(2):
         ...
         direc = norm.copy()
         assert abs(angle_offs[i]) < MAX_SEGMENT_ANGLE_OFFSET, "Offset angle is out of range."
         if angle_offs[i] != 0.0:
         ...
         assert -1.0 <= side_bias <= 1.0, "The side bias must be in range [-1.0, 1.0]."
         j = i + 2
         obj_verts[i].co = verts[i] + 0.5 * (side_bias - 1.0) * thickness * direc
         obj_verts[j].co = verts[i] + 0.5 * (side_bias + 1.0) * thickness * direc
...
class SegmentChain(Curve):
   def __init__(self, vertices: List[Vector], thickness: float = 0.05, side_bias: float = 0.0, 
                angle_offs0: float = 0.0, angle_offs1: float = 0.0, intro: Interval = None, 
                outro: Interval = None):
      super().__init__(vertices, thickness, side_bias, angle_offs0, angle_offs1, intro, outro)
